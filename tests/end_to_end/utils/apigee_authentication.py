...
def get_authentication_token(
    proxy_identifier: str, _request: pytest.FixtureRequest
) -> str:
    """Get the authentication token for the IM1 PFS Auth API.

    Args:
        proxy_identifier (str): The NHS number for the proxy
        _request (pytest.FixtureRequest): The pytest request fixture

    Returns:
        str: The authentication token.
    """
    # Apigee environment hardcoded as only mock only support composite tokens
    # on internal-dev now [2025-10-15]. The Apigee environment can be retrieved
    # from the fixture value "apigee_environment"
